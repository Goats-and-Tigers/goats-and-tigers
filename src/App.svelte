<script lang="ts">
  ...
  import type { PassiveTiles, AgressiveTiles, TileRow, TileCol } from "./tile";

  import { agressiveTiles, passiveTiles, playable_tiles } from "./tile";
  ...
  interface Tile {
    id: string;
    tile: string;
    tile_type: PassiveTiles | AgressiveTiles | "";
    el?: HTMLDivElement;
    tile_color: Players | "";
  }
  interface Tile_Loc {
  ...
  function is_state(): {
    ...
    let orange_ag = tiles.orange.filter((t) =>
      agressiveTiles.includes(t.tile.tile_type)
    );
    ...
    if (orange_ag.length == 0 && orange_passive.length != 0) {
      state.state =
        "Stalemate there's no fair game with no orange agressive tiles.";
      state.stale = true;
      return state;
    }
    if (white_ag.length == 0 && white_passive.length != 0) {
      state.state =
        "Stalemate there's no fair game with no white agressive tiles.";
      state.stale = true;
      return state;
    }
  ...
  let game_state = {};
  function getToken(token_type: "" | PassiveTiles | AgressiveTiles) {
  ...
  function is_valid_move(from: Tile_Loc, to: Tile_Loc) {
    ...
    if (
      agressiveTiles.includes(from_tile.tile_type) &&
        ...
        if (
          ...
          if (task == "moving") {
            ...
            if (
              board[id].tile_type != "" &&
              board[id].tile_color != turn &&
              agressiveTiles.includes(selected.tile_type)
            ) {
              if (
                (turn == "w" &&
              ...
              if (!agressiveTiles.includes(selected.tile_type)) {
                add_log(
                  "passive",
                  selected.tile_type,
                  "cannot attack",
...
<main>
  ...
  <div class="flex">
    <div bind:this={boardElement} id="board-holder" class="h-[25vh] w-[45%]">
