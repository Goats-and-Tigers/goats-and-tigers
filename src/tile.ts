...
export const aggressiveTiles = ['tiger', 'bear', 'snake', 'mantis-shrimp', 'bird'];

export type TileRow = 'a' | 'b' | 'c' | 'd' | 'e' | 'f' | 'g' | 'h';
export type TileCol = 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8;
export function get_tile(color: 'w' | 'o', name: PassiveTiles | AggressiveTiles) {
	return assets + '/tiles/' + color + '-' + name + '.svg';
}
