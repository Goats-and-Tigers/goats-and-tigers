<script lang="ts">
	...
	let input;
	let game = false;
	let game_id: string | null = 'SAMPLE';
	let buttons = true;
	let join = false;
	let menu = true;
	let db_game: any;
	...
	if (browser) {
		board_state = JSON.parse(localStorage.getItem('board') as string);
		board_fen = localStorage.getItem('fen_state') || 'null w';
	}
	let this_turn: any = 'w';
	let board_win_state: any;
	onMount(async () => {
		...
		if (params.get('id')) {
			...
			db_game = db.get(game_id as string);
			menu = false;
		}
		...
		db_game.map().once((e: string) => {
			...
			if (board_fen.split(' ')[1].length >= 2 && board_fen.split(' ')[1][1] == 'r') {
...
{#if menu}
	...
	>
		...
		{/if}
		{#if join}
			<input type="text" bind:this={input} class="bg-blue p-1" placeholder="Game id" />
			<button class="m-2 bg-orange p-1">Join</button>
		{/if}
	</div>
{/if}
<p class="-z-10">
	You: {browser ? localStorage.getItem('you') || this_turn : this_turn}
</p>
<p class="-z-10">
	current turn: {browser
		? board_fen.split(' ').length == 2
			? board_fen.split(' ')[1][0]
			: ''
		: ''}
</p>
<button
	on:click={() => {
		...
		localStorage.setItem('board', JSON.stringify(parse_fen('GHTBLMRS/8/8/8/8/8/8/ghtblmrs w')));
		localStorage.setItem('fen_state', 'GHTBLMRS/8/8/8/8/8/8/ghtblmrs w');
		db.get(game_id).set(localStorage.getItem('fen_state') + 'r');
		menu = true;
	}}>Resign</button
>
<Board multi board={board_state} turn={this_turn} game_state={board_win_state} />
